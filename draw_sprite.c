...
{
    // sort sprite byt distance
    int distance[mlx->count_sprite];
    int i = 0;
    while (i < mlx->count_sprite)
    {
        distance[i] = (player->pos_x - mlx->sprite[i].x) * (player->pos_x - mlx->sprite[i].x) + (player->pos_y - mlx->sprite[i].y) * (player->pos_y - mlx->sprite[i].y);
        i++;
    }
    i = 0;
    int tmp = 0;
    ...
    while (i < mlx->count_sprite)
    {
        double sprite_x = mlx->sprite[i].x - player->pos_x;
        double sprite_y = mlx->sprite[i].y - player->pos_y;
